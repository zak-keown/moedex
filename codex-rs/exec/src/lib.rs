...
enum StdinPromptBehavior {
    ...
    /// `moedex exec` behavior for `moedex exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `moedex exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("codex_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set moedex exec originator override {err:?}"
        );
    }
    ...
    if worktree {
        ...
        match command.as_ref() {
            Some(ExecCommand::Resume(_)) => {
                anyhow::bail!("--worktree is not supported with `moedex exec resume`");
            }
            Some(ExecCommand::Review(_)) => {
                anyhow::bail!("--worktree is not supported with `moedex exec review`");
            }
