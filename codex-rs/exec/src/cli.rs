...
#[command(
    version,
    override_usage = "moedex exec [OPTIONS] [PROMPT]\n       moedex exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Moedex outside a Git repository.
    ...
    /// Do not load config.toml from the effective Moedex home; auth still uses that home.
