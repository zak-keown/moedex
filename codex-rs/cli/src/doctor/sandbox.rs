...
#[cfg(target_os = "windows")]
const WINDOWS_SETUP_REMEDIATION: &str = "run moedex sandbox setup --elevated --user <end-user> --codex-home <authoritative-home> from an elevated shell";
                ...
                if let Some(report) = report {
                    let remediation = match report.code {
                        ...
                        | SetupErrorCode::HelperSandboxLockFailed => {
                            "rerun elevated setup for the authoritative Moedex home or ask IT"
                        }
                        ...
                        | SetupErrorCode::HelperFirewallRuleVerifyFailed => {
                            "ask IT to allow Moedex sandbox rules in managed Windows Firewall policy"
                        }
                        SetupErrorCode::OrchestratorPayloadSerializeFailed
                        | SetupErrorCode::HelperRequestArgsFailed
                        | SetupErrorCode::HelperUnknownError => {
                            "repair or reinstall the Moedex CLI from an approved distribution"
                        }
                    };
                    check
                        .details
