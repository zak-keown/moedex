...
//! interactive-session layers. Direct `moedex login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `moedex-login.log` artifact that
...
/// Installs a small file-backed tracing layer for direct `moedex login` flows.
///
...
/// command produce a durable `moedex-login.log` artifact without coupling it to the TUI's broader
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    let log_path = log_dir.join(format!("{}-login.log", PRODUCT_IDENTITY.executable_name));
    ...
    // Direct `moedex login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `{} login --device-auth` instead.",
...
/// `moedex login` working in environments where device-code may be disabled/feature-gated.
