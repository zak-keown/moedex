...
pub struct Cli {
    ...
    // Internal controls set by the top-level `moedex resume` subcommand.
    ...
    /// top-level `moedex resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `moedex fork` subcommand.
    // These are not exposed as user flags on the base `codex` command.
    #[clap(skip)]
    ...
    /// top-level `moedex fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
