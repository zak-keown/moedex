...
impl ExperimentalFeaturesView {
    ...
    fn header(&self, width: u16) -> impl Renderable {
        ...
        for text in [
            "Checked features are configured on. Some experimental features take effect only in new tasks or after restarting the Moedex server.",
    ...
    fn pre_draw_tick(&mut self, _now: Instant) -> bool {
        ...
        match result {
            ...
            Err(error) => {
                ...
                self.discovery_status = "Server experiments unavailable. Reopen /experimental to retry; restart this Moedex client if requests remain unanswered.".to_string();
            }
        }
        true
    }
