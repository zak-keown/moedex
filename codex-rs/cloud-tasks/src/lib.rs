...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    let auth = match auth {
        ...
        None => {
            eprintln!(
                "Not signed in. Please run 'moedex login' to sign in with ChatGPT, then re-run 'moedex cloud'."
            );
            std::process::exit(1);
        }
    };
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `moedex cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `moedex cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    ...
    if let Some(cursor) = page.cursor {
        let command = format!("moedex cloud list --cursor='{cursor}'");
        if colorize {
            println!(
...
/// Entry point for the `moedex cloud` subcommand.
