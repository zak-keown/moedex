...
(
  cd "$cargo_root"
  cargo run -p codex-cli --bin moedex -- exec-server --listen "$listen_url"
...
for _ in $(seq 1 "$((start_timeout_seconds * 20))"); do
  ...
  if ! kill -0 "$server_pid" >/dev/null 2>&1; then
    ...
    echo "failed to start moedex exec-server" >&2
    exit 1
  fi
...
if [[ -z "$exec_server_url" ]]; then
  cat "$stderr_log" >&2 || true
  cat "$stdout_log" >&2 || true
  echo "timed out waiting ${start_timeout_seconds}s for moedex exec-server to report its websocket URL" >&2
  exit 1
fi
